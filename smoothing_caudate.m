%This script includes a function with the group (which can be Controls, 1, or Patients, 2) as parameter, and computes the smoothing of the right and left caudate of each subject in the group. It runs in Matlab SPM. The image we want to smooth needs to be unzipped.

function F_smoothing9_Huntington(G)
...
if G==1
    for k = 1:length(pilots) %For all folders with subjects data
        ...
        left = fullfile(directory,pilots(k).name,'Caudate_masks', 'FA_transformation','FA_left_caudate.nii'); %path where the left caudate we want to smooth is stored
        right = fullfile(directory,pilots(k).name,'Caudate_masks', 'FA_transformation','FA_right_caudate.nii'); %path where the right caudate we want to smooth is stored
        
        %Perform left caudate smoothing
        ...
        %Perform right caudate smoothing
...
elseif G==2
    for k = 1:length(pilots) %For all folders with subjects data
        ...
        left = fullfile(pilots(k).name, 'Caudate_masks', 'FA_transformation','FA_left_caudate.nii'); %path where the left caudate we want to smooth is stored
        right = fullfile(pilots(k).name, 'Caudate_masks', 'FA_transformation','FA_right_caudate.nii'); %path where the right caudate we want to smooth is stored
