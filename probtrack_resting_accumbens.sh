        ...
        for side in left right #for each side of the brain
            ...
            for cortical in Executive Motivational Motor #for each cortical region
                ...
                for exclusion in Exclusion No_exclusion;
                    ...
                    mkdir "$DIR/Tractography_${group}_resting/$name/$exclusion/accumbens"

                    mkdir "$DIR/Tractography_${group}_resting/$name/$exclusion/accumbens/$cortical" #folder that will contain probtrackx output of each cortical region of the brain

                    mkdir "$DIR/Tractography_${group}_resting/$name/$exclusion/accumbens/$cortical/$side" #folder that will contain probtrackx output of each side of the brain


                    bedpost_dir="$DIR/Bedpost_${group}/$name/dmri.bedpostX" #directory where the bedpostX analysis is kept
                    ...
                    then 
                        ...
                        /usr/share/fsl/5.0/bin/probtrackx2 -x $DIR/$group/$name/accumbens_masks/FA_transformation/smoothed_${side}_accumbens.nii.gz -l --onewaycondition -c 0.2 -S 2000 --steplength=0.5 -P 5000 --fibthresh=0.01 --distthresh=0.0 --sampvox=0.0 --avoid="$DIR/$group/$name/Exclusion_masks/exclusion_mask_total_${cortical}_${side}_resting.nii.gz" --forcedir --opd -s $bedpost_dir/merged -m $bedpost_dir/nodif_brain_mask --dir=$DIR/Tractography_${group}_resting/$name/$exclusion/accumbens/$cortical/$side/seed2 --waypoints=$DIR/$group/$name/Cortical_masks_resting/$cortical/${cortical}_${side}_FA.nii.gz --waycond=AND

                    else #run the analisys without adding the two cortical masks not used as seed as exclusion masks
