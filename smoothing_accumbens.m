%This script includes a function with the group (which can be Controls, 1, or Patients, 2) as parameter, and computes the smoothing of the right and left accumbens of each of the subjects in the group. It runs in Matlab SPM. The image we want to smooth needs to be unzipped.

function F_smoothinga_Huntington(G)
...
if G==1
    for k = 1:length(pilots) %For all folders with subjects data
        ...
        left = fullfile(directory,pilots(k).name,'accumbens_masks', 'FA_transformation','FA_left_accumbens.nii'); %path where the left accumbens we want to smooth is stored
        right = fullfile(directory,pilots(k).name,'accumbens_masks', 'FA_transformation','FA_right_accumbens.nii'); %path where the right accumbens we want to smooth is stored
        
        %Perform left accumbens smoothing
        ...
        %Perform right accumbens smoothing
...
elseif G==2
    for k = 1:length(pilots) %For all folders with subjects data
        ...
        left = fullfile(pilots(k).name, 'accumbens_masks', 'FA_transformation','FA_left_accumbens.nii'); %path where the left accumbens we want to smooth is stored
        right = fullfile(pilots(k).name, 'accumbens_masks', 'FA_transformation','FA_right_accumbens.nii'); %path where the right accumbens we want to smooth is stored
